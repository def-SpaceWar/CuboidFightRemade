class ScreenObject {
    ...
    draw() {
        ...
        if (this.scale) {
            if (((this.x + camera.x) * camera.w_scale) - WIDTH * (camera.w_scale - 1) / 2 > WIDTH ||
                ((this.x + camera.x) * camera.w_scale) - WIDTH * (camera.w_scale - 1) / 2 < (0 - this.w * camera.w_scale)) {
                // x is off-screen

                if (((this.y + camera.y) * camera.h_scale) - HEIGHT * (camera.h_scale - 1) / 2 > HEIGHT ||
                    ((this.y + camera.y) * camera.h_scale) - HEIGHT * (camera.h_scale - 1) / 2 < (0 - this.h * camera.h_scale)) {
                    // y is off-screen
