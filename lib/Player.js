...
class Player {
        ...
        for (let i = 0; i < this.otherPlayers.length; i++) {
            ...
            if (distance <= this.attackRange * this.attackRange) {
                ...
                if (otherplayer.health.health <= 0) {
                    GameConsole.log(`<span style="color: ${otherplayer.color};">[Player ${otherplayer.playerNum}]</span> was killed by <span style="color: ${this.color};">[Player ${this.playerNum}]</span>!`);
                }
    ...
    updatePhysics(platforms, powerUps) {
        ...
        if (this.y > HEIGHT * 3 && this.health.health > 0) {
            ...
            GameConsole.log(`[Player ${this.playerNum}] Has Died!`, this.color)
        }
...
class PlayerHealth {
    ...
    constructor(health, maxHealth, color, borderColor, parentNum) {
        ...
        this.absorbtionScreenObject = new ScreenObject(
            this.x + this.w,
            this.y,
            0,
            "#ff0"
...
function regeneration(time, level) {
    ...
    return (player) => {
        GameConsole.log(`[Player ${player.playerNum}] Effect Regeneration { time: ${time}, level: ${level} }`, player.color)
...
function fasterAttack(time, level) {
    return (player) => {
        GameConsole.log(`[Player ${player.playerNum}] Effect AttackSpeed { time: ${time}, level: ${level} }`, player.color)
...
function moreDamage(time, level) {
    return (player) => {
        GameConsole.log(`[Player ${player.playerNum}] Effect DamageBoost { time: ${time}, level: ${level} }`, player.color)
