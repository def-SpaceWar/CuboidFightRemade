    ...
    loadSettings() {
        ...
        switch (this.class) {
            ...
            case "Warrior":
                ...
                this.damage *= 1.3;
                this.jumpPower /= 1.75;
                this.health.health = 110;
                this.health._health = 110;
                this.health.max_health = 110;
                this.kbMult = 1.3;
                this.kbDefence = 1.5;
                break;
            default:
    ...
    killBuff() {
        switch (this.class) {
            ...
            case "Ninja":
                this.attackCooldown /= 2;
                this.moveSpeed *= 1.3;
                this.effectors.push((player) => {GameConsole.log(`<span style="color: ${this.color};">[Player ${this.playerNum}]</span> Kill Buff Begin [`, "#FFFF00");});
                this.effectors.push(regeneration(10, 4));
                this.effectors.push((player) => {GameConsole.log(`<span style="color: ${this.color};">[Player ${this.playerNum}]</span> Kill Buff End ]`, "#FFFF00");});
                this.maxJumps += 1;
                break;
            case "Warrior":
                this.damage *= 1.15;
            ...
            case "lol":
                this.damage *= 100;
                this.health.modHealth(1000);
                this.attackCooldown = 0;
                GameConsole.debug("lol");
                break;
            default:
                this.effectors.push((player) => {GameConsole.log(`<span style="color: ${this.color};">[Player ${this.playerNum}]</span> Kill Buff Begin [`, "#FFFF00");});
                this.effectors.push(regeneration(3, 5));
                this.effectors.push(attackSpeed(3, 2));
    ...
    updatePhysics(platforms, powerUps) {
        ...
        if (this.health.health > 0) {
            ...
            if ((this.y > HEIGHT * 4 || this.y < -HEIGHT || this.x < -WIDTH || this.x > WIDTH * 4) && this.health.health > 0) {
                this.health.modHealth(-this.health.health, "void");
            }
        }
