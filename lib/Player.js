...
class Player {
    ...
    updatePhysics(platforms, powerUps) {
        ...
        for (let i = 0; i < platforms.length; i++) {
            if ((platforms[i].screenObjects[0].isCollided(this.screenObject) || platforms[i].screenObjects[1].isCollided(this.screenObject)) && this.ySpeed >= 0
...
class PlayerHealth {
    ...
    constructor(health, maxHealth, color, inactiveColor, borderColor, parent) {
        ...
        if (this.parentNum > 2) {
            this.comboObject.y = this.y - this.h / 2 - this.borderMargin * 2;
            this.killObject.y = this.y - this.h - this.borderMargin * 2;
